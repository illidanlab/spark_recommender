...
object RecOutput {
  ...
  def main(args: Array[String]) {
    ...
    val paraSc: SparkContext = sc
    val K = 3
    val programScorePaths = sc.textFile(inputPath).collect.toList
    doRecommendation(paraSc, date, roviPath, programScorePaths, outputPath, K)
  }
