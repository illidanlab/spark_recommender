...
import org.apache.spark.mllib.optimization.FeatureLearningFactorizationMachineRegressionModel
import org.apache.spark.mllib.optimization.FactorizationMachineRegressionFeatureLearningWithSGD

object RegressionModelFeatureLearningFactMachine 
	extends ModelProcessingUnit with 
		RegCustomizedModel[FeatureLearningFactorizationMachineRegressionModel] {
	
    def ParamLatentFactor = "latentFactor"
    def ParamL21Norm = "l21Param"
    def ParamL1Norm  = "l1Param"
    def ParamLearnedFeatureNum = "learnFeatureNum"
	...
	def learnModel(modelParams:HashMap[String, String], allData:AssembledDataSet, splitName:String, jobInfo:RecJob):ModelResource = {
        ...
        val l1Param   = modelParams.getOrElseUpdate(ParamL1Norm, "0.01").toDouble
        val ParamLearnedFeatureNum = 
            		 modelParams.getOrElseUpdate(RegressionModelFeatureLearningFactMachine.ParamLearnedFeatureNum, (latentDim * 2).toString).toInt
        ...
        if (jobInfo.outputResource(modelFileName)){
	        ...
	        val trainMeth = (input:RDD[LabeledPoint], numIterations:Int, stepSize:Double, regParam:Double) => 
	        					FactorizationMachineRegressionFeatureLearningWithSGD.train(
	        					        	input, latentDim, l21Param, l1Param, ParamLearnedFeatureNum, 
	        					        	numIterations, stepSize, regParam,  0.01)
	        ...
	        val modelStruct:CustomizedModelStruct[FeatureLearningFactorizationMachineRegressionModel] = 
	             new CustomizedModelStruct[FeatureLearningFactorizationMachineRegressionModel](IdenPrefix, resourceIden, 
	            		                            dataResourceStr,
	...
	val IdenPrefix:String = "RegModelRMF_FeatureLearning"
